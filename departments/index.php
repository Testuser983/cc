...
if(isset($_POST['submit']))
{
 ...
 if($captcha!=$captcharandom)
	...
	{
			$ret=mysqli_query($con,"SELECT * FROM department WHERE deptEmail='$email' and password='$password'");
			$num=mysqli_fetch_array($ret);

			if($num>0)
			{
				$extra="dashboard.php";
				$_SESSION['d_login']=$_POST['username'];
				$_SESSION['d_id']=$num['id'];
				$host=$_SERVER['HTTP_HOST'];
				$uip=$_SERVER['REMOTE_ADDR'];
				$status=1;
				$log=mysqli_query($con,"insert into deptlog(Did,Dept_name,Dept_ip,status) values('".$_SESSION['d_id']."','".$_SESSION['d_login']."','$uip','$status')");
			...
			else
			{
				...
				$uip=$_SERVER['REMOTE_ADDR'];
				$status=0;
				mysqli_query($con,"insert into deptlog(Dept_name,Dept_ip,status) values('".$_SESSION['d_login']."','$uip','$status')");
...
if(isset($_POST['change']))
{
   $email=$_POST['email'];
    ...
    $password=md5($_POST['password']);
$query=mysqli_query($con,"SELECT * FROM department WHERE deptEmail='$email' and contactNo='$contact'");
$num=mysqli_fetch_array($query);
if($num>0)
{
mysqli_query($con,"update departmnet set password='$password' WHERE deptEmail='$email' and contactNo='$contact' ");
...
<html lang="en">
  <head>
    ...
    <title>CCP | Department Login</title>
...
.captcha
{
  ...
  <body>
      ...
      *********************************************************************************************************************************************************** -->
	  ...
	  <div id="login-page">
	  	<div class="container">
	  		<h3 align="center" style="color:#fff"><a href="../index.html" style="color:#fff;text-transform:uppercase;"><B>Citizen Complaint portal</B></a></h3>
	<hr />
...
echo htmlentities($msg);
		        ...
		        <div class="login-wrap">
					<input type="text" class="form-control" name="username" placeholder="Dept ID"  required autofocus>
		            <br>
		            <input type="password" class="form-control" name="password" required placeholder="Password">
					<br>

				<div class="col-md-6 form-group">
				<input type="text" name="captcha"  placeholder="Enter Captcha" required class="form-control"/>
